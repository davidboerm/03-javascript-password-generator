...
function getPasswordOptions() {
  ...
  var length = parseInt(
    window.prompt("Desired password length? (min 8, max 128)"));

  // Conditional statement to check if pw length is a number
  if (isNaN(length) === true) {
    alert("Password length must be input as a number");
    return;
  }
  ...
  var useSpecial = window.confirm("Click OK to include special characters or Cancel to exclude");
  var useNumbers = window.confirm("Click OK to include numbers or Cancel to exclude");
  var useLowercase = window.confirm("Click OK to include lowercase letters or Cancel to exclude");
  var useUppercase = window.confirm("Click OK to include uppercase lettters or Cancel to exclude");
...
// function to write password in the target HTML element
